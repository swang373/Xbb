...
print "norm=",norm
for i in LO:
    count = 1.*CountWeightedLHEWeightPdf.GetBinContent(CountWeightedLHEWeightPdf.FindBin(i))
    tree.Draw("ZvvBDT >> histo%s (40,-1,1)"%i,"sign(genWeight)*LHE_weights_pdf_wgt[%s]*1./%s"%(i,count),"goff")
...
histo.GetYaxis().SetTitle("# pdf syst")
