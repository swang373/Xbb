...
tree->Draw("met_pt >> metDistr(300,0,300)","met_genPt>150 && met_genPt<170");
...
float maxim = max(max(metDistr->GetMaximum(),mhtRes->GetMaximum()),max(metPuppiRes->GetMaximum(),metType1p2Res->GetMaximum()));
